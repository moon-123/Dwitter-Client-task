...
const Tweets = memo(({ tweetService, username, addable }) => {
  ...
  useEffect(() => {
    ...
    const stopSyncDeleted = tweetService.onTweetDeleted((deletedTweetId) => {
    setTweets((tweets) => tweets.filter((tweet) => tweet.id !== deletedTweetId))
    })

    const stopSyncUpdated = tweetService.onTweetUpdated((updatedTweet) => {
      setTweets((tweets) => tweets.map((tweet) => tweet.id === updatedTweet.id ? updatedTweet : tweet))
    })
